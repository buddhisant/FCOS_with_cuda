...
class FCOSInference():
    ...
    def compute_results(self, cls_preds, reg_preds, cen_preds, points, ori_img_shape, fin_img_shape):
        """
        ...
        :param points: 根据feature map的size计算出的预设的anchor point
        ...
        for cls_pred, reg_pred, cen_pred, point in zip(cls_preds, reg_preds, cen_preds, points):
            ...
            bboxes = utils.ltrb2xyxy(point, reg_pred, fin_img_shape)
    ...
    def __call__(self, cls_preds, reg_preds, cen_preds, ori_img_shapes, fin_img_shapes):
        """
        ...
        :param fin_img_shapes: 输入图片放缩之后的大小
        :return:
        """
        ...
        points=utils.compute_points(feature_map_shapes, device, dtype) #获取预设的anchor point
        ...
        for i in range(batch_size):
            ...
            result_cls_cur_img, result_reg_cur_img, result_label_cur_img = self.compute_results(
                cls_preds_cur_img, reg_preds_cur_img, cen_preds_cur_img, points, ori_img_shapes[i], fin_img_shapes[i]
            )
