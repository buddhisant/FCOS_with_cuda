...
class Compose():
    ...
    def __call__(self, image, bboxs=None, labels=None):
        for t in self.transforms:
            image, bboxs, labels = t(image, bboxs, labels)
...
class Resize(object):
    ...
    def __call__(self, image, bboxs=None, labels=None):
        ...
        return resized_img, bboxs, labels
...
def build_transforms(is_train=True):
    ...
    toTensor=ToTensor()
    if is_train:
        transforms=Compose([resize, randomHorizontalFlip, normalize, toTensor])
    else:
        transforms=Compose([resize, normalize, toTensor])
    return transforms
